#!/usr/bin/env python3
"""
Simple WebSocket client (no external deps) to test ws://server/path
Usage: python tools/ws_test.py <host> <port> <path>
Example: python tools/ws_test.py 192.168.178.58 80 /ws
"""
import sys
import socket
import base64
...
if len(sys.argv) < 4:
    print("Usage: python tools/ws_test.py <host> <port> <path>")
    sys.exit(2)
...
try:
    ...
    payload = b"PING_FROM_TEST_CLIENT"
    fin_opcode = 0x81
    plen = len(payload)
    if plen <= 125:
    ...
    print("Sent masked text frame: ", payload)

    # receive one frame from server
    # read 2 bytes
    h = s.recv(2)
    if not h or len(h) < 2:
        print("No response frame")
    else:
        b1, b2 = h[0], h[1]
        ...
        if masked_flag and mask_key:
            ...
            data = bytes(unmasked)
        print("Received frame, opcode=", b1 & 0x0F, "payload:", data)
