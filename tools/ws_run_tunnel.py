#!/usr/bin/env python3
"""
Connect to ws://host:port/path, send a masked text frame with payload "TUNNEL,50",
then print incoming text frames until completion or timeout.
Usage: python tools/ws_run_tunnel.py <host> <port> <path>
"""
import sys, socket, base64, os, struct, time

if len(sys.argv) < 4:
    print("Usage: python tools/ws_run_tunnel.py <host> <port> <path> [count]")
    sys.exit(2)

host = sys.argv[1]
port = int(sys.argv[2])
path = sys.argv[3]
# optional count (defaults to 5)
count = 5
if len(sys.argv) >= 5:
    try:
...
try:
    ...
    # send TUNNEL command as masked text
    payload = ("TUNNEL,%d" % count).encode('ascii')
    fin_opcode = 0x81
    plen = len(payload)
    if plen <= 125:
