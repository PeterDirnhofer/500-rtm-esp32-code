...
esp_err_t saveStringParameter(const char* label, const char* value){
 ...
 // Write
    err = nvs_set_str(my_handle,label,value);
    if (err != ESP_OK) return err;
...
esp_err_t readStringParameter(const char* label, char* ptr){
    ...
    err = nvs_get_str(my_handle, label, NULL, &required_size);
    ...
    if (required_size == 0) {
        ...
        char outValue[10]={0};
        err = nvs_get_str(my_handle,label,outValue,&required_size);
        if (err != ESP_OK) {
        ...
        else printf("label %s found: ",label);
        ...
        //char res[required_size];
        //const char retstring[required_size+1]={0};
