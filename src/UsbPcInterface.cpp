...
static const char *TIP_ERROR_MESSAGE =
    "Invalid format 'TIP' command. \nSend 'TIP,10000,20000,30000' to set "
    "X,Y,Z\n'TIP,?' to see actual X Y Z values\n";

UsbPcInterface::UsbPcInterface() : mTaskHandle(NULL), mStarted(false) {
...
void UsbPcInterface::start() {
  ...
  xTaskCreatePinnedToCore(this->mUartRcvLoop, "uartRcvLoop", 10000, NULL, 4,
                          &this->mTaskHandle, (BaseType_t)0);
...
void UsbPcInterface::mUartRcvLoop(void *unused) {
  ...
  while (1) {
    // Read data from UART
    const int rxCount = uart_read_bytes(UART_NUM_1, data, RX_BUF_SIZE,
...
int UsbPcInterface::send(const char *fmt, ...) {
  ...
  // Also forward to WiFi interface if active
  if (WifiPcInterface::isActive()) {
    WifiPcInterface::send("%s", s);
  }
  va_end(ap);
